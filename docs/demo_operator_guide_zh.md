...
- 左侧是棋盘。
- 右侧是道路块网格预览、按钮、Debug 面板、Editor 面板。
- 顶部是关卡、燃料、当前状态。
- 棋盘上方的大号收益栏会显示当前 `Cash` 和 `Win +$N`，表示如果现在通关，本关会获得的 Cash。
...
- 每放置一次道路块，消耗 `1` 点燃料。
- 燃料归零前没有连到终点，则失败。
- 通关时，场上每 `1` 个道路格奖励 `10` Cash。
- 起点和已铺出的道路会计入通关奖金，终点不计入。
- 每次成功铺路后，棋盘上方收益栏里的 `Win +$N` 会实时更新。
- 连接到终点后会弹出胜利面板。
- 胜利面板会显示道路格数量、本关获得 Cash、当前总 Cash，并提供 `Next Level` / `Restart` / `Close` 按钮。
- 现金格可以通过直接铺上道路被收集。
...
7. 如果关卡需要 `3x3` 道路扩张触发，再放置压路机。
8. 如果关卡需要移动阻挡，再放置鼹鼠。
9. 点击 `Save Level`。
10. 按 `E` 退出编辑器。
11. 试玩关卡。
12. 根据体验调整燃料、障碍、现金、传送门、压路机、鼹鼠。
13. 再次保存。
