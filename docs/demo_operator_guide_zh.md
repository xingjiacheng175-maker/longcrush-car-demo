...
- `Jump To Level` 下拉框 + `Load Level`：直接加载指定关卡。
- `Next Level`：胜利后进入下一关。
- `D`：显示或隐藏 Debug 信息。
- `E`：进入或退出关卡编辑器。
...
- 燃料归零前没有连到终点，则失败。
- 现金格可以通过直接铺上道路被收集。
- 现金格也可以在接触到起点连通道路网络时被收集。
- 被收集的现金格会变成道路，并继续扩展连通网络。
- 障碍格不能铺道路。
- 传送门目前只有 `Portal A` 一组。
- 一个关卡里要么不放传送门，要么放两个 `Portal A`。
- 当起点连通道路网络连接到其中一个 `Portal A` 时，另一个 `Portal A` 也会被视为已经连通。
- 玩家可以从被连通的另一个传送门继续向外铺路。
...
- 现金和障碍不能覆盖起点或终点。
- 传送门不能覆盖起点或终点。
- `Portal A` 必须正好两个才合法；如果只画一个，不能保存。
...
4. 放置障碍，限制玩家路径。
5. 放置现金，鼓励玩家绕路。
6. 如果关卡需要传送连接，再放置两个 `Portal A`。
7. 点击 `Save Level`。
8. 按 `E` 退出编辑器。
9. 试玩关卡。
10. 根据体验调整燃料、障碍、现金、传送门。
11. 再次保存。
